...
import Graphics.Vty.Output
  ( DisplayContext (inlineHack),
    Mode (..),
    Output (..),
  )
import Graphics.Vty.Platform.Windows.Input.Focus (disableFocusEvents, requestFocusEvents)
import Graphics.Vty.Platform.Windows.Input.Mouse (disableMouseEvents, requestMouseEvents)
import qualified Graphics.Vty.Platform.Windows.Output.TerminfoBased as TerminfoBased
import Graphics.Vty.Platform.Windows.WindowsInterfaces (writeBuf)
...
reserveTerminal variant outHandle colorMode = liftIO $ do
  let flushedPut = void . writeBuf outHandle
  ...
  t <- TerminfoBased.reserveTerminal variant' outHandle colorMode
